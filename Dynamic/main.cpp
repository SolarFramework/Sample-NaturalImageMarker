...
float size_x = 0.285f;
float size_y = 0.197f;
float size_z = -0.3* (size_x + size_y );
...
int main(int argc, char *argv[])
{
	...
	while (true)
	{
		...
		if (isTrack)
		{
            ...
            computeCameraPose(pts3D, curPts, rvec, tvec, inliers);
		...
		{
			img_ar = frame.clone();
			visualAR(img_ar, rvec, tvec, cameraMatrix, distCoeffs);
			cv::imshow("AR", img_ar);
		}
...
void getProj(cv::Mat &proj)
{
    ...
    cv::Mat ext = (cv::Mat_<double>(3, 3) << rotW2C.at<double>(0, 0), rotW2C.at<double>(0, 1), tvec.at<double>(0, 0),
